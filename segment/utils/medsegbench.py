"""MedSegBench dataset definitions and download helpers."""

# pip install medsegbench
# https://github.com/zekikus/MedSegBench/blob/main/medsegbench/dataset.py
# category='data name'
from torch import from_numpy
from torch.utils.data import Dataset

# train_dataset = WbcMSBench(split="train", category='C1',size=256)
...
# download=True
# train_dataset = FlexibleMedSegBench(split="train", flag=data_name,download=download,size=256)
# test_dataset = FlexibleMedSegBench(split="test", flag=data_name, download=download,size=256)
# val_dataset = FlexibleMedSegBench(split="val", flag=data_name, download=download,size=256)
...
def get_MedSegBench_dataset(flag,transform,split, download=True, size=256):
    ...
    获取指定数据集的实例
    :param flag: 数据集标识符
    :param split: 数据集划分（train/test/val）
    :param download: 是否下载数据集
    :param size: 图像大小
    :return: 数据集实例
    """
