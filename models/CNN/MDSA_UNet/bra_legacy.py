...
class Aggregator(nn.Module):
    def __init__(self, dim, seg=4):
        ...
        self.out_proj = nn.Conv2d(self.dim, self.dim, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.out_proj(x).permute(0, 2, 3, 1)
...
class TopkRouting(nn.Module):
    ...
    def forward(self, query:Tensor, key:Tensor)->Tuple[Tensor]:
        ...
        query_hat = F.normalize(query_hat, p=2.0, dim=-1)
        key_hat = F.normalize(key_hat, p=2.0, dim=-1)
        attn_logit = -xy_pairwise_distance(query_hat, key_hat)
        # attn_logit = (query_hat*self.scale) @ key_hat.transpose(-2, -1) # (n, p^2, p^2)
        topk_attn_logit, topk_index = torch.topk(attn_logit, k=self.topk, dim=-1) # (n, p^2, k), (n, p^2, k)
        r_weight = self.routing_act(topk_attn_logit) # (n, p^2, k)

        # import numpy as np
        # np.array(attn_logit[0])
...
class QKVLinear(nn.Module):
    def __init__(self, dim, qk_dim, bias=True):
        ...
        # self.qkv = nn.Linear(dim, qk_dim + dim, bias=bias)

    def forward(self, x):
        # qk, v = self.qkv(x).split([self.qk_dim, self.dim], dim=-1)
        # q = k = qk
        # kv = torch.cat([k, v], dim=-1)
        q = x
        kv = torch.cat([x, x], dim=-1)
        return q, kv
