...
class SafeVRWKVSpatialMix(nn.Module):
    """RWKV-6 style spatial mix without the CUDA WKV extension or token limit."""
    ...
    def __init__(self, n_embd, channel_gamma=1 / 4, shift_pixel=1):
        ...
        # Low-Rank Dimension for Dynamic LERP (ddlerp)
        self.dynamic_dim = max(32, n_embd // 8)
        
        # Low-rank projection to compute dynamic mixing weights for w, k, v, r, g
        self.spatial_maa_x = nn.Linear(n_embd, self.dynamic_dim * 5, bias=False)
        self.spatial_maa_w = nn.Linear(self.dynamic_dim, n_embd, bias=False)
        self.spatial_maa_k = nn.Linear(self.dynamic_dim, n_embd, bias=False)
        self.spatial_maa_v = nn.Linear(self.dynamic_dim, n_embd, bias=False)
        self.spatial_maa_r = nn.Linear(self.dynamic_dim, n_embd, bias=False)
        self.spatial_maa_g = nn.Linear(self.dynamic_dim, n_embd, bias=False)
        ...
        self.gate = nn.Linear(n_embd, n_embd, bias=False) # Gate layer from RWKV-6
    ...
    def _init_weights(self):
        # Learnable static bias mixing parameters
        self.spatial_mix_w = nn.Parameter(torch.ones([1, 1, self.n_embd]) * 0.5)
        self.spatial_mix_k = nn.Parameter(torch.ones([1, 1, self.n_embd]) * 0.5)
        self.spatial_mix_v = nn.Parameter(torch.ones([1, 1, self.n_embd]) * 0.5)
        self.spatial_mix_r = nn.Parameter(torch.ones([1, 1, self.n_embd]) * 0.5)
        self.spatial_mix_g = nn.Parameter(torch.ones([1, 1, self.n_embd]) * 0.5)
    ...
    def forward(self, x, patch_resolution=None):
        if self.shift_pixel > 0:
            ...
            # Compute dynamic mixing coefficients (ddlerp)
            sx = torch.tanh(self.spatial_maa_x(x)) # Shape: (B, T, dynamic_dim * 5)
            sw, sk, sv, sr, sg = torch.chunk(sx, 5, dim=-1)
            
            mw = self.spatial_maa_w(sw)
            mk = self.spatial_maa_k(sk)
            mv = self.spatial_maa_v(sv)
            mr = self.spatial_maa_r(sr)
            mg = self.spatial_maa_g(sg)
            
            xw = x + xxx * (self.spatial_mix_w + mw)
            xk = x + xxx * (self.spatial_mix_k + mk)
            xv = x + xxx * (self.spatial_mix_v + mv)
            xr = x + xxx * (self.spatial_mix_r + mr)
            xg = x + xxx * (self.spatial_mix_g + mg)
        else:
            xw = xk = xv = xr = xg = x
        ...
        g = torch.clamp(torch.nan_to_num(self.gate(xg), nan=0.0, posinf=30.0, neginf=-30.0), -30.0, 30.0)

        # Safe non-sequential approximation of WKV
        x_wkv = self.key_norm(k + v)
        # Apply output gating
        x_gated = (torch.sigmoid(r) * x_wkv) * torch.nn.functional.silu(g)
...
class RWKV_UNetV4(nn.Module):
    """RWKV_UNet variant with RWKV-6-style safe spatial mixing without compiling legacy WKV op."""
    ...
    def __init__(self, input_channel=3, num_classes=1, img_size=256, pretrained_path=""):
        ...
        if pretrained_path:
            raise ValueError(
                "RWKV_UNetV4 is standalone and does not load legacy RWKV_UNet pretrained weights."
            )
...
def rwkv_unetv4(input_channel=3, num_classes=1, img_size=256, **kwargs):
    return RWKV_UNetV4(
