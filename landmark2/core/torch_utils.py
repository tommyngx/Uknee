...
from landmark2.core.cpu import CPUInfo
...
def get_cpu_info():
    ...
    from landmark2.core import PERSISTENT_CACHE  # avoid circular import error
...
def parse_device(device: str | int | list | tuple | torch.device = "") -> str:
    ...
    if "-1" in device:
        from landmark2.core.autodevice import GPUInfo
...
def model_info_for_loggers(trainer):
    ...
    Args:
        trainer (landmark2.core.trainer.BaseTrainer): The trainer object containing model and validation data.
    ...
    if trainer.args.profile:  # profile ONNX and TensorRT times
        from landmark2.core.benchmarks import ProfileModels
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    Examples:
        >>> from pathlib import Path
        >>> from landmark2.core.torch_utils import strip_optimizer
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Examples:
        >>> from landmark2.core.torch_utils import profile_ops
