...
from landmark2.core.checks import check_file, check_font, is_ascii, normalize_platform_uri
from landmark2.core.downloads import download, safe_download, unzip_file
from landmark2.core.ops import segments2boxes
...
def merge_multi_segment(_segments):
    ...
    raise ValueError("landmark2 supports YOLO pose labels and does not convert COCO segmentation polygons")
...
def convert_ndjson_to_yolo_if_needed(data: str | Path) -> str | Path:
    ...
    if data_str.endswith(".ndjson") or (data_str.startswith("ul://") and "/datasets/" in data_str):
        raise ValueError("landmark2 accepts local YOLO dataset YAML files, not NDJSON or Platform URIs")
    return data
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not train_set.is_dir():
        ...
        if image_files := list(data_dir.rglob("*.jpg")) + list(data_dir.rglob("*.png")):
            from landmark2.data.split import split_classify_dataset
...
class HUBDatasetStats:
    ...
    Examples:
        >>> from landmark2.data.data_utils import HUBDatasetStats
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                from landmark2.data import YOLODataset
...
def compress_one_image(f: str, f_new: str | None = None, max_dim: int = 1920, quality: int = 50):
    ...
    Examples:
        >>> from pathlib import Path
        >>> from landmark2.data.data_utils import compress_one_image
