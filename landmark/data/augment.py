...
from landmark.data.data_utils import polygons2masks, polygons2masks_overlap
from landmark.core import LOGGER, IterableSimpleNamespace, colorstr
from landmark.core.checks import check_version
from landmark.core.instance import Instances
from landmark.core.metrics import bbox_ioa
from landmark.core.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
from landmark.core.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from landmark.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from landmark.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
...
class CutMix(BaseMixTransform):
    ...
    Examples:
        >>> from landmark.data.augment import CutMix
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV(BaseTransform):
    ...
    Examples:
        >>> import numpy as np
        >>> from landmark.data.augment import RandomHSV
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from landmark.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
class Format(BaseTransform):
    ...
    def _format_img(self, img: np.ndarray) -> torch.Tensor:
        ...
        convert_to_rgb = img.shape[0] == 3 and (
            self.bgr <= 0.0 or (self.bgr < 1.0 and random.uniform(0, 1) > self.bgr)
        )
        img = np.ascontiguousarray(img[::-1] if convert_to_rgb else img)
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace):
    ...
    Examples:
        >>> from landmark.core.config import DEFAULT_CFG
        >>> from landmark.data.dataset import YOLODataset
        >>> from landmark.core import IterableSimpleNamespace
