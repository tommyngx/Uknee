...
from landmark.core.config import IterableSimpleNamespace
from landmark.data.dataset import (
    GroundingDataset,
...
from landmark.data.loaders import (
    LOADERS,
    LoadImagesAndVideos,
    LoadPilAndNumpy,
    LoadScreenshots,
...
from landmark.data.data_utils import IMG_FORMATS, VID_FORMATS
from landmark.core import RANK, colorstr
from landmark.core.checks import check_file
from landmark.core.torch_utils import TORCH_2_0
...
def build_yolo_dataset(
    ...
    fixed_rectangular_canvas = isinstance(cfg.imgsz, (list, tuple)) and len(cfg.imgsz) == 2 and cfg.imgsz[0] != cfg.imgsz[1]
    return dataset(
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
        augment=mode == "train",
        hyp=cfg,
        # Aspect-ratio batch sorting assumes one scalar canvas. A fixed HxW
        # canvas already preserves anatomy through LetterBox and needs no rect batching.
        rect=False if fixed_rectangular_canvas else (cfg.rect or rect),
