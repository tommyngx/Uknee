"""Load the pinned Ultralytics snapshot bundled with ``landmark``.

Ultralytics uses absolute ``ultralytics.*`` imports internally.  The vendored
directory is therefore added to the front of ``sys.path`` before any landmark
...
def load_vendored_ultralytics():
    ...
    if loaded is not None:
        ...
        if ULTRALYTICS_ROOT not in loaded_file.parents:
            raise RuntimeError(
                "A non-vendored 'ultralytics' package was imported before landmark: "
                f"{loaded_file}. Import landmark before ultralytics, or remove the "
    ...
    loaded_file = Path(getattr(loaded, "__file__", "")).resolve()
    if ULTRALYTICS_ROOT not in loaded_file.parents:
        raise RuntimeError(f"landmark resolved the wrong Ultralytics backend: {loaded_file}")
