...
`landmark0` is the archived standalone knee-landmark package. It vendors Ultralytics
8.4.87 under `_vendor/ultralytics`, so **do not install the `ultralytics` pip
package**. Importing `landmark0` bootstraps and verifies the local snapshot;
...
```bash
pip install -r landmark0/requirements.txt
```

Single process:

```bash
python -m landmark0.train \
  --model landmark0/cfg/models/yolo26-pose-v9.yaml \
  --data landmark0/cfg/datasets/mesko4gf2.yaml \
...
```bash
torchrun --standalone --nproc-per-node=2 -m landmark0.train \
  --model landmark0/cfg/models/yolo26-pose.yaml \
  --data landmark0/cfg/datasets/mesko4gf2.yaml \
  --batch 16 --device 0,1
```
...
```python
from landmark0 import KneePose

model = KneePose("landmark0/cfg/models/yolo26-pose.yaml")
model.train(data="landmark0/cfg/datasets/mesko4gf2.yaml", epochs=100, device=0)
metrics = model.val(data="landmark0/cfg/datasets/mesko4gf2.yaml")
...
`landmark0/utils`; `KneePose` remains importable directly from `landmark0`.
...
Every run is written to `runs/landmark0/<model>_<dataset>/`; `best.pt` is selected
