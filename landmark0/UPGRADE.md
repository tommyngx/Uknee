...
`landmark0/data/schema.py` và `_vendor/ultralytics/utils/oa26_region_refine/region_schema.py` **định nghĩa cùng** MESKO4GF2 schema:
...
2. `landmark0/cfg/default.yaml` — 45 dòng, override cho pose only
3. `_vendor/ultralytics/cfg/__init__.py` — config parser (`get_cfg`, `get_save_dir`...)

Model YAMLs cũng tách 3 nơi:
1. `landmark0/cfg/models/` — 5 YAML (yolo26-pose, hrnet, vitpose)
...
| File | Nội dung |
|---|---|
| `landmark0/utils/plotting.py` | `plot_dashboard_pose`, `plot_pose_metrics`, `plot_validation_samples` — 350 LoC |
...
| `landmark0/utils/validation.py` | `KneePoseValidator(PoseValidator)` — thêm MRE/PCK/HD95 |
...
`landmark0/heatmap/` chỉ có 2 files:
- `__init__.py` (6 LoC)
...
**→ Merge**: Dời `engine.py` → `landmark0/models/heatmap_adapter.py`. Xoá thư mục `heatmap/`.
...
│       │   ├── convnextv2_n.py
│       │   ├── convnextv2_t.py
│       │   ├── hrnet.py            #    OA26 HRNet backbone (khác với landmark0/models/hrnet.py)
│       │   ├── pose_heads.py
│       │   └── vit_refine.py
...
|---|---|---|
| Thư mục chính | 10 (`core`, `engine`, `nn`, `yolo`, `optim`, `cfg`, `data`, `heatmap`, `models`, `utils`) | **5** (`cfg`, `core`, `data`, `nn`, `models`) + `utils` (landmark0-specific) |
...
Sau đó **override** bằng giá trị từ `landmark0/cfg/default.yaml`:

```yaml
# OVERRIDE cho Uknee Pose
task: pose
seed: 2006            # thay vì 0
plots: false           # landmark0 tự render
save_period: -1
```
...
```text
cfg/models/
├── yolo26-pose.yaml          # ← giữ từ landmark0/cfg/models/
...
# === Từ landmark0/utils/plotting.py (medical dashboard) ===
def plot_dashboard_pose(...): ...
def plot_pose_metrics(...): ...
def plot_validation_samples(...): ...
```

**Lưu ý**: `landmark0/utils/results.py` hiện import từ `landmark0.utils.plotting` — sau merge, đổi thành import từ `landmark0.core.plotting`.
...
# Import đổi: ultralytics.* → landmark0.core.*
# Class: HeatmapPoseModel, HeatmapPose, HeatmapPoseTrainer, HeatmapPoseValidator, HeatmapPosePredictor
```

Xoá thư mục `landmark0/heatmap/` sau khi dời.

---

## 5. Import mapping tổng hợp

| Import cũ (`ultralytics.*`) | Import mới (`landmark0.*`) |
|---|---|
| `from ultralytics import YOLO` | `from landmark0.core.model import YOLO` |
| `from ultralytics.engine.model import Model` | `from landmark0.core.model import Model` |
| `from ultralytics.engine.trainer import BaseTrainer` | `from landmark0.core.trainer import BaseTrainer` |
| `from ultralytics.engine.validator import BaseValidator` | `from landmark0.core.validator import BaseValidator` |
| `from ultralytics.engine.predictor import BasePredictor` | `from landmark0.core.predictor import BasePredictor` |
| `from ultralytics.engine.results import Results` | `from landmark0.core.results import Results` |
| `from ultralytics.engine.exporter import Exporter` | `from landmark0.core.exporter import Exporter` |
| `from ultralytics.models.yolo.detect import DetectionTrainer` | `from landmark0.core.detect import DetectionTrainer` |
| `from ultralytics.models.yolo.pose import PoseTrainer, PoseValidator` | `from landmark0.core.pose import PoseTrainer, PoseValidator` |
| `from ultralytics.nn.tasks import ...` | `from landmark0.nn.tasks import ...` |
| `from ultralytics.cfg import get_cfg` | `from landmark0.core import get_cfg` |
| `from ultralytics.utils import LOGGER, RANK, ops, DEFAULT_CFG` | `from landmark0.core import LOGGER, RANK, DEFAULT_CFG` + `from landmark0.core.ops import ...` |
| `from ultralytics.utils.loss import PoseLoss26` | `from landmark0.core.loss import PoseLoss26` |
| `from ultralytics.utils.metrics import PoseMetrics` | `from landmark0.core.metrics import PoseMetrics` |
| `from ultralytics.utils.torch_utils import model_info` | `from landmark0.core.torch_utils import model_info` |
| `from ultralytics.utils.oa26.heatmap import ...` | `from landmark0.core.targets import ...` |
| `from ultralytics.utils.oa26.loss import ...` | `from landmark0.core.loss import ...` |
| `from ultralytics.utils.oa26.simcc import ...` | `from landmark0.core.targets import ...` |
| `from ultralytics.utils.oa26_region_refine.region_schema import ...` | `from landmark0.data.schema import ...` |
| `from ultralytics.utils.oa26_region_refine.loss import ...` | `from landmark0.core.loss import ...` |
| `from ultralytics.data import build_yolo_dataset` | `from landmark0.data.build import build_yolo_dataset` |
| `from ultralytics.data.augment import LetterBox` | `from landmark0.data.augment import LetterBox` |
| `from ultralytics.data.utils import check_det_dataset` | `from landmark0.data.data_utils import check_det_dataset` |
| `from ultralytics.optim import MuSGD` | `from landmark0.core.torch_utils import MuSGD` |
| `from ultralytics.utils.plotting import Annotator` | `from landmark0.core.plotting import Annotator` |
...
### Xoá thư mục landmark0 cũ

| Thư mục/File | Lý do |
|---|---|
| `landmark0/heatmap/` | Đã dời vào `models/heatmap_adapter.py` |
| `landmark0/utils/plotting.py` | Đã merge vào `core/plotting.py` |
| `landmark0/_vendor/` | Mục tiêu chính — xoá hoàn toàn |

### Ước tính giảm

| | Trước | Sau | Giảm |
|---|---|---|---|
| Python files | 208 (184 vendor + 24 landmark0) | ~55 | **~74%** |
...
| 1.10 | Test imports | `python -c "from landmark0.core import LOGGER, RANK"` |
...
| 3.5 | Tạo `core/plotting.py` ← merge vendor plotting + landmark0 plotting |
...
### Phase 5: Models adapter + landmark0 code update + cleanup
...
| 5.3 | Xoá `landmark0/utils/plotting.py` (đã merge) |
| 5.4 | Update `landmark0/__init__.py` — xoá bootstrap |
| 5.5 | Update tests |
| 5.6 | **Xoá `_vendor/`** |
| 5.7 | Xoá `landmark0/heatmap/` |
| 5.8 | Test toàn bộ |
...
```bash
# Sau mỗi phase:
python -m pytest landmark0/tests/ -v

# Smoke test:
python landmark0/train.py --model cfg/models/yolo26-pose-v1.yaml --data <dataset>.yaml --epochs 2 --batch 4 --device cpu
python landmark0/train.py --model cfg/models/hrnet-w32-pose.yaml --data <dataset>.yaml --epochs 2 --batch 4 --device cpu

# Export test:
python -c "from landmark0 import KneePose; m = KneePose('best.pt'); m.export(format='onnx')"
```
