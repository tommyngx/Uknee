...
from landmark0.data.prepare import PreparedDataset, prepare_dataset
from landmark0.utils.results import KneePoseResult, adapt_yolo_result
...
class KneePose:
    ...
    def __init__(self, model: str | Path, *, verbose: bool = False) -> None:
        ...
        if any(marker in lowered for marker in LEGACY_MODEL_MARKERS):
            raise ValueError(
                f"Legacy landmark0 checkpoint/config '{self.model_path.name}' is not compatible with the "
        ...
        if self.family == "heatmap":
            from landmark0.heatmap.engine import HeatmapPose
            ...
            try:
                ...
                raise
            if type(self.backend.model).__module__.startswith("landmark0.heatmap"):
                from landmark0.heatmap.engine import HeatmapPose
    ...
    def train(self, *, data: str | Path, **kwargs: Any):
        ...
        args.update(
            ...
            project=str(PACKAGE_ROOT.parent / "runs" / "landmark0"),
            name=run_name,
            exist_ok=True,
            plots=False,
            save_period=-1,
        ...
        if self.family == "yolo":
            from landmark0.utils.validation import KneePoseTrainer
    ...
    def val(self, *, data: str | Path, **kwargs: Any):
        ...
        prepared = self._prepare_data(data)
        if self.family == "yolo":
            from landmark0.utils.validation import KneePoseValidator
    ...
    def export(self, *, format: str = "onnx", **kwargs: Any):
        ...
        from landmark0.utils.exporting import KneePoseExportWrapper
